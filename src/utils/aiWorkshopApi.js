...
const AI_WORKSHOP_BASE_URL = `/api/workshop`;
...
export const uploadFile = async (workspaceId, file, onUploadProgress) => {
  ...
  formData.append('s3_folder', 'files'); // Specify the 'files' folder for S3
  return apiRequest('POST', `${AI_WORKSHOP_BASE_URL}/workspaces/${workspaceId}/files/`, formData, true, true, null, onUploadProgress);
};
...
export const aiChatWithWorkspace = async (workspaceId, data) => {
  return apiRequest('POST', `${AI_WORKSHOP_BASE_URL}/ai-chat/`, data, true);
};
