...
const StakeholderInterviews = () => {
  ...
  const get_step_id = async () => {
    ...
    try {
      ...
      message.error("Failed to load audit decision interview data");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!interviewText.trim()) {
      message.warning("Please provide details for the audit decision interview.");
      return;
    }
    setIsLoading(true);
    try {
      const formData = new FormData();
      formData.append("field_name", "Audit Decision Interview Details");
      ...
      if (response.status === 201) {
        message.success("Audit decision interview details submitted successfully!");
        setIsModalVisible(false);
        setInterviewText("");
        setFileList([]);
        setOldFilesNeeded([]);
        ...
        message.error("Failed to submit audit decision interview details.");
      }
    } catch (error) {
      message.error("Failed to submit audit decision interview details.");
      console.error(error);
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">Audit Decision Interviews</h2>
          ...
          <div className="p-6">
            ...
            <div className="space-y-4 mb-6">
              {interviewData.map((item) => (
                <div key={item.id} className="border-l-4 border-blue-500 bg-gray-50 rounded-r-lg">
      ...
      <Modal
        title={interviewData.length > 0 ? "Update Audit Decision Interview" : "Add Audit Decision Interview"}
        open={isModalVisible}
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 mb-6">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-600">
                  <p>Please download and fill in the template below before submitting your audit decision interview details.</p>
                </div>
                <div className="mt-3">
                  <a
                    href="/temp.txt"
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="interviewDescription" className="block text-sm font-medium text-gray-700 mb-2">
                Audit Decision Interview Details
              </label>
              <TextArea
                id="interviewDescription"
                value={interviewText}
                onChange={(e) => setInterviewText(e.target.value)}
                placeholder="Enter your audit decision interview details"
                rows={10}
