...
import IAPedit from "./IAPedit";

const IAPbody = ({ projectId, refreshProjectData }) => {
  ...
  // Fetch Internal Audit Process data (step_no = 9)
  const fetchStepData = async () => {
    ...
    try {
      ...
      // Fetch fresh data
      const response = await getStepDetails(currentProjectId, 9);
      ...
      console.error('Error fetching Internal Audit Process data:', error);
      message.error('Failed to load Internal Audit Process data');
    } finally {
      setIsLoading(false);
    }
  };
