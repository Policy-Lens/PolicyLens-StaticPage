...
const Vapt = ({ reportId, projectId, specificReportMode = false }) => {
  // PDF viewer state
  ...
  const fetchVaptReports = async () => {
    ...
    try {
      const res = await apiRequest(
        "GET",
        `/api/rarpt/project/${effectiveProjectId}/vapt/`,
        null,
        true
      );
      ...
      if (res.status === 200) {
        ...
        if (specificReportMode && effectiveReportId) {
          ...
          const specificReport = res.data.find(report => report.id === parseInt(effectiveReportId));
          if (specificReport) {
            setSelectedReport(specificReport);
            setPdfFile(specificReport.file);
            
            // Initialize viewer for specific report
            setTimeout(async () => {
              try {
                ...
                instance.current = await WebViewer(
                  {
                    ...
                    initialDoc: specificReport.file,
                  },
                  viewerDiv.current
                );
          ...
          // Normal mode - select the first report by default
          if (res.data.length > 0) {
            const reportToSelect = res.data[0];
  ...
  // Open edit modal with current report info
  const openEditModal = () => {
  ...
  const updateReport = async () => {
    ...
    try {
      ...
      if (editReportName.trim()) {
        formData.append("name", editReportName.trim());
      }
      if (editReportFile) {
        formData.append("file", editReportFile);
      }
      const response = await apiRequest(
        "PATCH",
        `/api/rarpt/vapt/${selectedReport.id}/update/`,
        formData,
        true
      );
      if (response.data) {
      ...
      console.error("Error updating VAPT report:", err);
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 p-2 bg-white sticky top-0 z-10 shadow-sm">
        <div className="flex items-center">
          <h2 className="text-xl font-bold text-slate-800">VAPT Reports</h2>
          ...
          {vapt_reports.length > 0 && !specificReportMode && (
        ...
        <div className="flex items-center gap-3">
          ...
          {selectedReport && (
            <>
              <button
                className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors shadow-md flex items-center"
            ...
            <button
              className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors shadow-md flex items-center"
              onClick={() => deleteSheet(selectedReport.id, selectedReport.name)}
              disabled={isLoading}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                        ...
                        <div className="flex text-sm text-gray-600">
                          ...
                          >
                            ...
                            <input
                              ...
                              className="sr-only"
                              accept=".pdf,.doc,.docx,.xls,.xlsx"
                              onChange={(e) => setEditReportFile(e.target.files[0])}
                            />
                          </label>
                          <p className="pl-1">or drag and drop</p>
                        </div>
                    ...
                    {editReportFile && (
                      <p className="mt-2 text-sm text-gray-500">
                        Selected file: {editReportFile.name}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-yellow-500 text-base font-medium text-white hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 sm:ml-3 sm:w-auto sm:text-sm disabled:bg-yellow-300"
                >
